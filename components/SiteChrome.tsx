  ...
  return (
    <>
      ...
      <header className="gtop">
        ...
        <a href="/" className="glogo" aria-label="GeekFon Society home"><img src="/geekfon-logo.png" alt="" /></a>
        <nav className="gcrumb" aria-label="Breadcrumb">
          {(crumb && crumb.length ? crumb : [{ label: "GeekFon", href: "/" }]).map((x, i, a) => (
            <span key={i}>
...
const CHROME_CSS = `
.gtop{position:sticky;top:0;z-index:40;height:60px;display:flex;align-items:center;gap:12px;padding:0 18px;background:#fff;border-bottom:1px solid var(--lr-border)}
.gham{width:40px;height:40px;border:none;background:none;cursor:pointer;display:flex;align-items:center;justify-content:center;border-radius:8px;color:var(--lr-text);flex-shrink:0}
...
.glogo{display:flex;align-items:center;flex-shrink:0}
.glogo img{height:30px;width:auto;display:block}
.gcrumb{display:flex;align-items:center;gap:9px;min-width:0;overflow:hidden;font-family:'Montserrat',sans-serif}
.gcrumb a,.gcrumb .gcrumb-cur{font-size:16px;font-weight:800;letter-spacing:.01em;color:var(--lr-text);white-space:nowrap;text-transform:none}
.gcrumb a:hover{color:var(--rx-text)}
.gcrumb .gcrumb-cur{color:var(--rx-text)}
.gcrumb-sep{color:var(--lr-text-30);font-weight:600}
@media(max-width:600px){.gcrumb a,.gcrumb .gcrumb-cur{font-size:13px}}
.gcta{margin-left:auto;flex-shrink:0;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.1em;color:var(--rx-text);border:1px solid var(--rx);border-radius:20px;padding:8px 17px;background:#fff}
...
.gitem{display:block;padding:12px 14px;border-radius:8px;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:rgba(255,255,255,.7)}
