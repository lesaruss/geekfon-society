...
// Renamed from /api/plus-apply, 2026-07-27 (see app/pro/page.tsx). Table name
// (gfs_plus_applications) and storage bucket (gfs-plus-applications) are left
// as-is - internal only, no UI reads them by name, and renaming would mean
// migrating already-uploaded applicant videos for zero user-facing benefit.
export async function POST(req: NextRequest) {
  try {
    ...
    if (error) {
      ...
      console.error("Pro apply insert error:", error);
      return NextResponse.json({ error: "Database error" }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("pro-apply route error:", err);
